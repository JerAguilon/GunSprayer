...
//gun data
var pattern = [[0,0],[-2,0],[-18,1],[-19,2],[-29,3],[-43,3],[-58,7],[-71,11],[-79,10],[-86,4],
...
for(i = 0; i < pattern.length;i++){
	for(j = 0;j<2;j++){
		pattern[i][j] = pattern[i][j] * normalizer
	}
}
...
var fireBullet = function(x,y){
	...
	if(firing){
		...
		context.arc((2.63/sensitivity*pattern[i][1]+originX), originY - (2.63/sensitivity*pattern[i][0]), radius, 0, Math.PI*2);
		context.fill();
